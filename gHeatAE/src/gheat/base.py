...
class ColorScheme(object):
  ...
  def __init__(self, name, fspath, store):
    ...
    self.build_empties(store)


  def build_empties(self, store):
...
#                raise AttributeError( "Permissions too restrictive on "
#                                        + "empties directory "
#                                        + "(%s)." % empties_dir
#                                         )
...
#                    os.remove(os.path.join(empties_dir, fname))
    for zoom, opacity in gheat.opacity.zoom_to_opacity.items():
      ...
      self.hook_build_empty(opacity, store)
...
class Store(object):
  """Base Class for representing storage of images
  """
  
  def get(self, key):
    """Get and return the image from a data store
    """
    raise NotImplementedError

  def set(self, key):
    """Set the image to the data store
    """
    raise NotImplementedError
...
class Tile(object):
  ...
  def __init__(self, color_scheme, dots, zoom, x, y, store):
